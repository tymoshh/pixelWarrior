    ...
    while(!quit) {
        ...
        for (bulletNs::bulletClass& bulletInstance : bulletNs::bulletVector) {
            ...
            if (!bulletInstance.isOnScreen()) {
                bulletNs::bulletsToDelete.push_back(bulletNs::bulletIterator);
            }
            bulletNs::bulletIterator++;
        ...
        bulletNs::bulletIterator = 0;
        for (int bulletIndexToDelete : bulletNs::bulletsToDelete) {
            bulletNs::bulletVector.erase(bulletNs::bulletVector.begin() + bulletIndexToDelete - bulletNs::bulletIterator);
            bulletNs::bulletIterator++;
        }
        ...
        for (zombieNs::zombieClass& zombieInstance : zombieNs::zombieVector) {
            ...
            if (zombieNs::isZombieDead) {
                zombieNs::zombiesToDelete.push_back(zombieNs::zombieIterator);
            }
        }
        // deleting zombies that are flagged to delete
        zombieNs::zombieIterator = 0;
        for (int zombieIndexToDelete : zombieNs::zombiesToDelete) {
            zombieNs::zombieVector.erase(zombieNs::zombieVector.begin() + zombieIndexToDelete - zombieNs::zombieIterator); // maybe a problem idk
            zombieNs::zombieIterator++;
